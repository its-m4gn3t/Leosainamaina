    ...
    <div className={`flex items-end gap-2 mb-2 ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'} ${isGrouped ? 'mt-1' : 'mt-4'}`}>
      ...
      <div className={`flex flex-col max-w-xs lg:max-w-md ${isOwnMessage ? 'items-end' : 'items-start'}`}>
        ...
        >
          ...
          {/* Message Actions */}
          {showActions && (
            <div className={`absolute top-0 ${isOwnMessage ? 'left-0 -translate-x-full' : 'right-0 translate-x-full'} flex items-center gap-1 bg-white shadow-lg rounded-lg p-1 border`}>
              <button
          ...
          {/* Reaction Picker */}
          {showReactions && (
            <div className="absolute top-full mt-1 bg-white shadow-lg rounded-lg p-2 border flex gap-1 z-10">
              {reactions.map(emoji => (
                <button
                  key={emoji}
                  onClick={() => {
